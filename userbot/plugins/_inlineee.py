...
if BOT_MSG is None:
    BOT_LIT = f"Hello Sir MySelf 𝐒𝐀𝐕𝐀𝐆𝐄 𝐒𝐀𝐕𝐀𝐆𝐄 Here For  {LIGHTNINGUSER}'s Protection "
else:
    BOT_LIT = BOT_MSG   
...
if LIGHTNING_BOT_PIC is None:
    LIGHTNING_WARNING = "https://telegra.ph/file/9b2f6cb7ed8037a8ef59f.jpg"
else:
    ...
    if lightning.query.user_id == bot.uid and query.startswith("**Black") or query.startswith("Black"):
        ...
        result = builder.article(
            ...
            buttons=[
                ...
                [Button.url("Commands Not Working🥺", "https://t.me/SAVAGE_TEAM_CLUB")],
                [Button.url("Help Article 🤓", "https://app.gitbook.com/@poxsisofficial/s/help/")],
                [
                    Button.url(
                
                    "Want To Learn CMDS😅",
                    "https://t.me/SAVAGE_TEAM_CLUB" ,
                    )
                ], 
            ],
        )
...
async def lightning_pugins_query_hndlr(lightning):
    ...
    try:
        if light_pulu_name in CMD_HELP:
           ...
           lightning_is_best += "\n\n**In Case Any Problem @SAVAGE_TEAM_CLUB** ".format(light_pulu_name)
...
In Case Any Problem @SAVAGE_TEAM_CLUB \nTottal Plugs( ͡🔥 ͜ʖ ͡🔥): {len(CMD_LIST)}"""
