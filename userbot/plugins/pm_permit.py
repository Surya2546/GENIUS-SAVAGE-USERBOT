...
if LIGHTNING_PM is None:
    CUSTOM_LIGHTNING_PM_PIC = ":https://telegra.ph/file/05397b1819c99e70ff052.jpg"
else:
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            ...
            await borg.send_message(
                chats, "**Alert! My dev @mahadevbambhole is here. **"
            )
            print("Dev is here")
...
async def krish_op(event):
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            lightning_sql.approve(chats.id, "**Heya Sir**")
            await borg.send_message(
                chats, f"**Good To See You @mahadevbambhole. How Can I Disapprove You Come In Sir**😄😄"
            )
            print("Dev Here")
@bot.on(
...
async def krish_op(event):
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            lightning_sql.approve(chats.id, "**Heya Sir**")
            await borg.send_message(
                chats, f"**Good To See You lipi @mahadevbambhole. How Can I Disapprove You Come In Sir**😄😄"
            )               
            print("Dev Here")
...
async def krish_op(event):
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            lightning_sql.approve(chats.id, "**Heya Sir**")
            await borg.send_message(
                chats, f"**GULSHAN IS HERE \n #GULSHAN IS HERE ATTENTION AUTO APPROVED**😄😄"
            )               
            print("DEV IS HERE")            
@bot.on(
...
async def krish_op(event):
    ...
    if event.is_private:
        if not lightning_sql.is_approved(chats.id):
            ...
            await borg.send_message(
                chats, f"`GULSHAN OP IZ HERE\n `@mahadevbambhole`IZ HERE 😅,How can I disapprove u sir ,SO A͛U͛T͛O͛ A͛P͛P͛R͛O͛V͛E͛D͛⚡🙃🙂🙃 ` "
            )               
            print("`GULSHAN OP IZ HERE ⚡`")            
